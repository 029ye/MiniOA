...
public class TopMenu {
	...
	private List<TopMenu> items;

	public List<TopMenu> getItems() {
		if (items == null)
			buildItems();
		return items;
	}
	...
	public void buildItems() {
		try {
			...
			while (it.hasNext()) {
				...
				items.add(new TopMenu(id, cId, cDate, mId, mDate, uuid, parent, name, url, target, seq));
				rowcount++;
			}
			it = null;
	 ...
	 * 树形菜单：菜单管理
	 */
	private TreeNode<TopMenu> rootNode = null;

	public TreeNode<TopMenu> getTreeNode() {
		if (rootNode == null)
			loadTree();
		return rootNode;
	}
