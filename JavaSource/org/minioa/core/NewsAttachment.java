...
public class NewsAttachment {
	...
	private List<NewsAttachment> items;

	public List<NewsAttachment> getItems() {
		if (items == null)
			buildItems();
		return items;
	}
	...
	public void buildItems() {
		try {
			getMySession();
			items = new ArrayList<NewsAttachment>();
			...
			while (it.hasNext()) {
				...
				p.put("uuid", uuid);
				items.add(new NewsAttachment(p));
			}
			it = null;
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public void buildSi() {
		try {
			...
			if (null == uuid)
				uuid = getMySession().getTempStr().get("NewsAttachment.uuid");
			Query query = getSession().getNamedQuery("core.news.attachment.records.images");
			query.setParameter("uuid", uuid);
			Iterator<?> it = query.list().iterator();
			String url = "";
			while (it.hasNext()) {
				...
				url = url.replaceAll("\\\\", "/");
				url = url.substring(url.indexOf("upload"));
				si.add(new SelectItem("../../" + url, FunctionLib.getString(obj[9])));
			}
			it = null;
		} catch (Exception ex) {
			ex.printStackTrace();
