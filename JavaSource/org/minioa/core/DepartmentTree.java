...
public class DepartmentTree {
	...
	public void processSelection(NodeSelectedEvent event) {
		...
		if (bean.getType().equals("depa")) {
			getMySession().getTempInt().put("DepartmentTree.id", bean.getID_());
			getMySession().getTempStr().put("DepartmentTree.depaName", bean.getDepaName());
		} else {
			getMySession().getTempInt().put("DepartmentTree.id", 0);
			getMySession().getTempStr().put("DepartmentTree.depaName", "");
		}
	}
